...
b);PhotoWall._el=b.el+" .body";PhotoWall._c_width=$(PhotoWall._el).width();PhotoWall._c_height=$(PhotoWall._el).height();PhotoWall._line_max_height=PhotoWall.options.lineMaxHeight;PhotoWall._setLineMaxHeightDynamic();$(PhotoWall._el).html("").addClass("clearfix");$(window).resize(PhotoWall.RESIZE)},_init_socials:function(){var b=document,c,d=b.getElementsByTagName("script")[0],e=function(e,g){if(!b.getElementById(g))c=b.createElement("script"),c.src=e,c.id=g,d.parentNode.insertBefore(c,d)};e("https://connect.facebook.net/en_US/all.js#xfbml=1",
"fbjssdk");e("https://apis.google.com/js/plusone.js","gplus1js");e("https://platform.twitter.com/widgets.js","tweetjs")},_setLineMaxHeightDynamic:function(){if(PhotoWall.options.lineMaxHeightDynamic){var b=$(window).height()/PhotoWall.options.baseScreenHeight;PhotoWall.options.lineMaxHeight=100<=PhotoWall._line_max_height*b?PhotoWall._line_max_height*b:100}},RESIZE:function(){var b=$(PhotoWall._el).width(),c=$(PhotoWall._el).height();PhotoWall._setLineMaxHeightDynamic();if(!(!PhotoWall.options.lineMaxHeightDynamic&&
PhotoWall._c_width==b||PhotoWall._block_resize))ShowBox.opened()?PhotoWall._must_resize=!0:(PhotoWall._c_width=b,PhotoWall._c_height=c,PhotoWall._must_resize=!1,$(PhotoWall._el).html(""),PhotoWall.show())},load:function(b){if(!PhotoWall.options.lineMaxHeightDynamic)for(var c in b)b[c].th.width=Math.round(b[c].th.width*(PhotoWall.options.lineMaxHeight/b[c].th.height)),b[c].th.height=PhotoWall.options.lineMaxHeight;PhotoWall._photos=b;PhotoWall.show()},show:function(b){var c=[],d=[],e=0;if(b)c=b,d=
PhotoWall._last_line[0],e=PhotoWall._last_line[1];else{if(!PhotoWall._photos)return;$(PhotoWall._el).html("");$(window).scrollTop(0);c=PhotoWall._photos}var h=function(b,c,e,d,g,f,h,l){var j="",k="";h&&l&&(j="position:absolute;left:-"+Math.round((e-h)/2)+"px;top:-"+Math.round((d-l)/2)+"px;",k="pw-crop");b=$('<div id="'+b+'" class="pw-photo '+k+' clearfix" style="margin:'+c+"px;width:"+e+"px;height:"+d+'px;float:left;"><a class="pw-link" href="'+g+'"><img class="pw-zoom" src="'+f+'" width="'+e+'" height="'+
d+'" style="'+j+'" /></a></div>');$.browser.msie?(b.find("img").hide(),PhotoWall._inited&&b.find("img").load(function(){$(this).fadeIn(300)})):(b.find("img").css("opacity",0),PhotoWall._inited&&b.find("img").load(function(){$(this).delay(700*Math.random()+300).animate({opacity:1},{duration:1E3})}));return b},b=function(b,c,e,d){var g=$("<div class='line' style='float:left'></div>").appendTo(PhotoWall._el),f=b.length,d=d?PhotoWall._c_width*PhotoWall.options.firstBigWidthPercent+2*PhotoWall.options.padding:
0,c=(PhotoWall._c_width-d-2*f*PhotoWall.options.padding)/c;e&&(c=1);var l=0,j;for(j in b){var k=Math.round(b[j].th.width*c),m=Math.round(b[j].th.height*c),l=l+k;!e&&j==f-1&&(k+=PhotoWall._c_width-d-2*f*PhotoWall.options.padding-l);k=h(b[j].id,PhotoWall.options.padding,k,m,b[j].img,b[j].th.src);g.prepend(k)}return k},g=0,f=2*PhotoWall.options.padding,j=!0,k;for(k in c){var n=0,m=!1;if(PhotoWall.options.lineMaxHeightDynamic){var l=PhotoWall.options.lineMaxHeight/c[k].th.height;c[k].th.width=Math.round(c[k].th.width*
...
c[k].width,c=f.width*l,l*=f.height,f.th.width=c,f.th.height=l,f.th.src=f.img,f.th.zoom_src=f.img,PhotoWall._first_big=h(f.id,PhotoWall.options.padding,c,l,f.img,f.img).prependTo(PhotoWall._el);if(d)2>g&&PhotoWall.options.isFirstBig&&(m=!0),l=b(d,e,1,m),PhotoWall._last_line=[d,e,l];if(!PhotoWall._inited)PhotoWall._inited=!0,PhotoWall.RESIZE(),PhotoWall.initGUI()},initGUI:function(){PhotoWall.options.zoom&&PhotoWall.initZoom();PhotoWall.options.showBox&&PhotoWall.initShowBox()},initShowBox:function(){var b=
"",c=null;PhotoWall.options.showBoxSocial&&(b='<div style="padding-top: 5px;width: 250px;position: relative;left: 50%;margin-left: -125px;"><div style="float:left;margin-top: 5px;width:80px;"><div id="gplus" class="g-plusone" data-size="medium"></div></div><div style="float:left;margin-top:5px;width:90px;"><a href="https://twitter.com/share" class="twitter-share-button">Tweet</a></div><div style="float:left;margin-top:5px;width:80px;" id="fblike"><fb:like send="false" layout="button_count" width="100" show_faces="false"></fb:like></div></div>',
