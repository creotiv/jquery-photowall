...
c=function(){"undefined"!==typeof FB&&FB.XFBML.parse(document.getElementById("fblike"));"undefined"!==typeof gapi&&gapi.plusone.render(document.getElementById("gplus"),{href:location.href,annotation:"bubble",width:90,align:"left",size:"medium"});"undefined"!==typeof twttr&&($("#showbox .twitter-share-button").attr("data-url",location.href),twttr.widgets.load())});ShowBox.init(PhotoWall._el+" a.pw-link",{closeCallback:function(){PhotoWall._must_resize&&PhotoWall.RESIZE()},menuBarContent:b,onUpdate:c});
PhotoWall.options.showBoxSocial&&PhotoWall._init_socials()},initZoom:function(){$(".pw-zoom").live(PhotoWall.options.zoomAction,function(){var b=$(this);if(parseInt(b.css("opacity")))b.stop().addClass("pw-photo-hover"),PhotoWall._zoom_timeout=setTimeout(function(){b.removeClass("pw-photo-hover");b.addClass("pw-photo-zoomed");if(PhotoWall._zoom_timeout){PhotoWall._zs=[b.width(),b.height()];var c=b.parent().parent(),d=PhotoWall._photos[c.attr("id")],e=$("<img/>");e.attr("src",d.th.zoom_src);e.load(function(){b.attr("src",
$(this).attr("src"));$(this).remove()});var e=Math.round(b.width()*d.th.zoom_factor),d=Math.round(b.height()*d.th.zoom_factor),h=e-b.width(),g=d-b.height(),h=0.5*-h-PhotoWall.options.zoomImageBorder,g=0.5*-g-PhotoWall.options.zoomImageBorder,c=c.offset(),f=$(window),j=1;c.left+h+e>f.width()+f.scrollLeft()&&(h-=c.left+h+e-(f.width()+f.scrollLeft())+2*PhotoWall.options.zoomImageBorder);c.left+h<f.scrollLeft()&&(h-=c.left+h-f.scrollLeft());c.top+g+d>f.height()+f.scrollTop()&&(g-=c.top+g+d-(f.height()+
f.scrollTop())+2*PhotoWall.options.zoomImageBorder);c.top+g<f.scrollTop()&&(g-=c.top+g-f.scrollTop());e+2*PhotoWall.options.zoomImageBorder>f.width()&&(j*=(f.width()-2*PhotoWall.options.zoomImageBorder)/(e+2*PhotoWall.options.zoomImageBorder));d*j+2*PhotoWall.options.zoomImageBorder>f.height()&&(j*=(f.height()-2*PhotoWall.options.zoomImageBorder)/(d*j+2*PhotoWall.options.zoomImageBorder));b.animate({"margin-left":h,"margin-top":g,width:Math.round(e*j),height:Math.round(d*j),padding:PhotoWall.options.zoomImageBorder},
{queue:!1,duration:PhotoWall.options.zoomDuration,easing:"linear"})}},PhotoWall.options.zoomTimeout)});$(".pw-zoom").live("mouseleave",function(){var b=$(this);b.parent().parent().attr("id");if(PhotoWall._zoom_timeout)clearTimeout(PhotoWall._zoom_timeout),PhotoWall._zoom_timeout=null;b.removeClass("pw-photo-hover");if(b.hasClass("pw-photo-zoomed"))b.stop().css({margin:"",padding:"",width:PhotoWall._zs[0]+"px",height:PhotoWall._zs[1]+"px"}),PhotoWall._zs=null,b.removeClass("pw-photo-zoomed")})}},ShowBox=
{version:"0.1.4",_opened:!1,_preview_locked:!1,_images:[],_index:0,_current:"",_inited:!1,_th:null,options:{closeCallback:function(){},menuBarContent:"",onUpdate:null},init:function(b,c){ShowBox.options=$.extend(ShowBox.options,c);ShowBox._init(b);ShowBox._initEvents(b);ShowBox._parseGet()},_init:function(b){ShowBox._images.push([]);var c=ShowBox.options.menuBarContent;ShowBox.options.menuBarContent&&(c='<div class="showbox-menubar unselect" unselectable="on" style="display:none !important;">'+ShowBox.options.menuBarContent+
"</div>");ShowBox._inited||$('<div id="showbox" style="display:none;">    <div id="showbox-exit"></div>'+c+'    <div class="showbox-image unselect" unselectable="on">    </div>    <div id="showbox-loader"></div>    <div class="showbox-preview unselect">        <div class="showbox-pv-lock"></div>        <div class="showbox-th-counter" unselectable="on"></div>    </div></div>').appendTo("body");$("body").append('<div id="showbox-thc'+(ShowBox._images.length-1)+'" style="overflow:hidden;width:100%;position:absolute;top:-999999px;"><div class="showbox-th-container clearfix"></div></div>');
...
ShowBox.options.closeCallback();$("#showbox .showbox-menubar").hide();$("#showbox .showbox-image img").remove();$("#showbox-thc"+ShowBox._current).css({position:"absolute",top:"-10000px"});$("#showbox-thc"+ShowBox._current).detach().appendTo("body")},KEYPRESSED:function(b){27==b.keyCode&&ShowBox.EXIT();39==b.keyCode&&ShowBox._next();37==b.keyCode&&ShowBox._prev()},OPENPREVIEW:function(b){$(b).animate({bottom:0},{queue:!1,duration:150})},CLOSEPREVIEW:function(b){$(b).animate({bottom:-75},{queue:!1,
duration:150})},LOCKPREVIEW:function(b){ShowBox._preview_locked=ShowBox._preview_locked?!1:!0;ShowBox._preview_locked?($("#showbox .showbox-pv-lock").show(),$(b).css({background:"#111","border-top":"1px solid #383838"})):($("#showbox .showbox-pv-lock").hide(),$(b).css({background:"","border-top":""}))},RESIZE:function(){if(ShowBox._opened){var b=$("#showbox"),c=$("#showbox .showbox-image img"),d=b.width(),e=b.height()-146;ShowBox.options.menuBarContent||(e=b.height()-111);var b=parseInt(c.attr("height")),
